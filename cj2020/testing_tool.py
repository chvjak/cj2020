...
class JudgeSingleCase(object):
  ...
  def Judge(self):
    ...
    for i in range(MAX_QUERIES + 1):
      ...
      print("Contestant input: {0}".format(contestant_input), file=sys.stderr)
      if err is not None:
        return err
      ...
      if i % 10 == 0:
        ...
        if random.randint(0, 1):
          print("Mirroring", file=sys.stderr)
          self.arr = Reverse(self.arr)
        if random.randint(0, 1):
          print("Negating", file=sys.stderr)
          self.arr = BitFlip(self.arr)
        print("Result: {0}".format(self.arr ), file=sys.stderr)
